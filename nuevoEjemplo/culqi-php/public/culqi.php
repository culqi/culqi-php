...
class UrlAESCipher {
    ...
    public function urlBase64Encrypt($sp755f74) {
      if ($this->sp42f8e2()) {
        ...
        return trim(UrlAESCipher::base64_encode_url($sp1c42c6 .
        mcrypt_encrypt($this->cipher, $this->key, $sp810031, $this->mode, $sp1c42c6)));
      }
    ...
    public static function pkcs5_unpad($sp2dbdda) {
       ...
       if ($spf187c8 > strlen($sp2dbdda)) { return false; }
       if (strspn($sp2dbdda, chr($spf187c8), strlen($sp2dbdda) - $spf187c8) != $spf187c8) { return false; }
  ...
  class Culqi {
    public static $llaveSecreta; public static $codigoComercio;
    public static $servidorBase = 'https://pago.culqi.com';

    public static function cifrar($sp3231b0) {
      $spadece1 = new UrlAESCipher(); $spadece1->setBase64Key(Culqi::$llaveSecreta);
  ...
  class Pago {
    ...
    public static function crearDatospago($sp821fb9, $sp37cd46 = null){
      ...
      $spa944fe = array(Pago::PARAM_COD_COMERCIO => Culqi::$codigoComercio,
      Pago::PARAM_INFO_VENTA => $sp327f8d);

      $spd4bb0a = Pago::validateAuth($spa944fe);

      if (!empty($spd4bb0a) && array_key_exists(Pago::PARAM_TICKET, $spd4bb0a)) {
        $spd3ecee = array(Pago::PARAM_COD_COMERCIO => $spd4bb0a[Pago::PARAM_COD_COMERCIO],
        Pago::PARAM_TICKET => $spd4bb0a[Pago::PARAM_TICKET]);
        $spd4bb0a[Pago::PARAM_INFO_VENTA] = Culqi::cifrar(json_encode($spd3ecee));
      }
    ...
    public static function consultar($sp517f27) {
      ...
      $sp821fb9 = array(Pago::PARAM_COD_COMERCIO => Culqi::$codigoComercio,
      Pago::PARAM_INFO_VENTA => $sp327f8d);
      return Pago::postJson(Culqi::$servidorBase . Pago::URL_CONSULTA, $sp821fb9);
    }

    public static function anular($sp517f27) {
      $sp327f8d = Pago::getCipherData(array(Pago::PARAM_TICKET => $sp517f27));
      $sp821fb9 = array(Pago::PARAM_COD_COMERCIO => Culqi::$codigoComercio, Pago::PARAM_INFO_VENTA => $sp327f8d);
      return Pago::postJson(Culqi::$servidorBase . Pago::URL_ANULACION, $sp821fb9);
    }

    private static function getCipherData($sp821fb9, $sp37cd46 = null) {
      $sp4c1514 = array_merge(array(Pago::PARAM_COD_COMERCIO => Culqi::$codigoComercio), $sp821fb9);

      if (!empty($sp37cd46)) { $sp4c1514[Pago::PARAM_EXTRA] = $sp37cd46; }

      $sp4c1514[Pago::PARAM_SDK_INFO] = Pago::getSdkInfo();
      $spe828da = json_encode($sp4c1514); return Culqi::cifrar($spe828da);
    }

    private static function validateAuth($sp821fb9) {
      return Pago::postJson(Culqi::$servidorBase . Pago::URL_VALIDACION_AUTORIZACION, $sp821fb9);
    }
    ...
    private static function validateParams($sp821fb9) {
      if (!isset($sp821fb9[Pago::PARAM_MONEDA]) or empty($sp821fb9[Pago::PARAM_MONEDA])) {
        ...
        if (!isset($sp821fb9[Pago::PARAM_MONTO]) or empty($sp821fb9[Pago::PARAM_MONTO])) {
          ...
          if (is_numeric($sp821fb9[Pago::PARAM_MONTO])) { if (!ctype_digit($sp821fb9[Pago::PARAM_MONTO])) {
