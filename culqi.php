...
define("CULQI_SDK_VERSION", "1.0.0");
...
class Pago {
    ...
    const URL_VALIDACION_AUTORIZACION = "/web/validar/";
    const URL_ANULACION = "/anular/";
    const URL_CONSULTA = "/consultar/";
    ...
    public static function crearDatospago($params, $extra = null) {
        ...
        if (!empty($response) && array_key_exists(Pago::PARAM_TOKEN, $response)) {
            $infoVenta = array(
                Pago::PARAM_COD_COMERCIO => $response[Pago::PARAM_COD_COMERCIO],
                Pago::PARAM_TOKEN => $response[Pago::PARAM_TOKEN]
            );
